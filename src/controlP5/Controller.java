...
public abstract class Controller<T> implements ControllerInterface<T>, CDrawable, ControlP5Constants {
    ...
    protected Controller(final ControlP5 theControlP5, final ControllerGroup<?> theParent, final String theName, final float theX, final float theY, final int theWidth, final int theHeight) {
        ...
        positionBuffer.setLocation(theX, theY);
    ...
    protected Controller(final String theName, final float theX, final float theY) {
        _myName = theName;
        position.setLocation(theX, theY);
    }
    ...
    @Override
    @ControlP5.Layout
    public T setPosition(Point2D thePos) {
        position = thePos.clone();
        positionBuffer = thePos.clone();
        return me;
    }
    ...
    @Override
    @ControlP5.Invisible
    public T setAbsolutePosition(Point2D thePos) {
        // TODO
        // doesnt work properly yet.
        // absolute position should not be changed from the
        // outside anyway.
        absolutePosition.setLocation(thePos.x, thePos.y);
        return me;
    }
    ...
    public final T setParent(final ControllerInterface<?> theParent) {
        ...
        absolutePosition.setLocation(position.x, position.y);
        if (theParent != null) {
            _myParent = theParent;
            _myParent.add(this);
            absolutePosition.setLocation(absolutePosition.x + _myParent.getPosition().x, absolutePosition.y + _myParent.getPosition().y);
