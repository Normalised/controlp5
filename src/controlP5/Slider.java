...
public class Slider extends Controller<Slider> {
    ...
    public Slider(ControlP5 theControlP5, ControllerGroup<?> theParent, String theName, float theMin, float theMax, float theDefaultValue, float theX, float theY, int theWidth, int theHeight) {
        ...
        mousePosForShiftScaling = new Point2D();
        mousePosRelative = new Point2D();
    ...
    public void keyEvent(KeyEvent theEvent) {
        if (isMousePressed && theEvent.isShiftDown() && (theEvent.getAction() == KeyEvent.PRESS) && !shiftIsDown) {
            ...
            // otherwise the slider will just jump to the cursor position
            if (isMousePressed) {
                // For now pretend that the mouse has been released until I find a good way of consistenly handling relative mouse control
    ...
    protected void mousePressed() {
        //cp5.println("Mouse Pressed");
        mousePosRelative.setLocation(0f, 0f);
    }
    ...
    private abstract class SliderView implements ControllerView<Slider> {
        ...
        abstract void drawValueRect(PGraphics theGraphics);

        void setSnapValue() {
            float n = PApplet.round(PApplet.map(_myValuePosition, 0, getSize(), 0, _myTickMarks.size() - 1));
            _myValue = PApplet.map(n, 0, _myTickMarks.size() - 1, _myMin, _myMax);
        }

        protected void updateUnit() {
            _myUnit = (_myMax - _myMin) / (getSize() - _myHandleSize);
        }

        protected void setAndMapValue(float f) {
            setValue(PApplet.map(f, 0, 1, _myMinReal, _myMaxReal));
        }
        ...
        public void display(PGraphics theGraphics, Slider theController) {
            ...
            drawValueRect(theGraphics);
            if (isLabelVisible) {
                drawCaptionAndValueLabels(theGraphics, theController);
            }
            if (isShowTickMarks) {
                drawTickMarks(theGraphics);
            }
        }

        void drawCaptionAndValueLabels(PGraphics theGraphics, Slider theController) {
        ...
        private void drawTickMarks(PGraphics theGraphics) {
            float x;
            boolean vertical = getDirection() == VERTICAL;

            theGraphics.pushMatrix();
            if (vertical) {
                theGraphics.translate(-4, (getSliderMode() == FIX) ? 0 : getHandleSize() / 2);
                x = ((float) getHeight() - ((getSliderMode() == FIX) ? 0 : getHandleSize())) / (getTickMarks().size() - 1);
            } else {
                theGraphics.translate((getSliderMode() == FIX) ? 0 : getHandleSize() / 2, getHeight());
                x = ((float) getWidth() - ((getSliderMode() == FIX) ? 0 : getHandleSize())) / (getTickMarks().size() - 1);
            }
            ...
            for (TickMark tm : getTickMarks()) {
                theGraphics.pushMatrix();
                offset = PApplet.round(x * tickMarksCount);

                if (tickMarksCount < totalTickMarks - 1) {
                    theGraphics.translate(vertical ? 0 : offset, vertical ? offset : 0);
                } else {
                    theGraphics.translate(vertical ? 0 : offset - 1, vertical ? offset - 1 : 0);
                }
    ...
    private class SliderViewV extends SliderView {
        ...
        void update() {
            float localY = _myParent.getAbsolutePosition().y + position.y;
            float f = _myMin + (-(_myControlWindow.mouseY - localY - getHeight())) * _myUnit;
            setAndMapValue(f);
        }

        void drawValueRect(PGraphics theGraphics) {
    ...
    private class SliderViewH extends SliderView {
        ...
        void update() {
            if (!shiftIsDown) {
                float localX = _myParent.getAbsolutePosition().x + position.x - mousePosRelative.x;
                float dx = (_myControlWindow.mouseX - localX);
                //cp5.println(localX + " : " + _myControlWindow.mouseX + " : " + mousePosRelative.x);
